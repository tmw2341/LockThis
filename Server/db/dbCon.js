...
function addUser(name, email, username, pass, callback) { //Add user to database
    getDB(db => db.run(`INSERT INTO Users(Name, Email, Username, Password) VALUES(?, ?, ?, ?)`, [name, email, username, pass], function (err) {
        if (err) {
...
function addPerm(user, lock, callback) { //adds permission to database
    getDB(db => db.run(`INSERT INTO Permissions(LockID, UserID) VALUES(?, ?)`, [lock, user], function (err) {
        if (err) {
            console.log(err.message);
            return callback(false);
        }
